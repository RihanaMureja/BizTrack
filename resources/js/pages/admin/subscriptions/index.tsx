...
import { Check, CreditCard, Pencil, Power, Save } from 'lucide-react';
...
export default function AdminSubscriptionsIndex({
    ...
    const submit = (event: FormEvent) => {
        ...
        const url = editing
            ? `/admin/subscriptions/${form.data.id}`
            : '/admin/subscriptions';
    ...
    const edit = (subscription: Subscription) =>
        form.setData({
            ...
            max_cashiers: String(subscription.max_cashiers),
    ...
    const columns: DataTableColumn<Subscription>[] = [
        ...
        },
        {
            key: 'price',
            header: 'Price',
            render: (plan) => `${Number(plan.price).toFixed(2)} ETB`,
        },
                ...
                <div className="flex justify-end gap-2">
                    ...
                    {plan.status === 'active' ? (
                        <IconButton
                            ...
                            onClick={() =>
                                router.post(
                                    `/admin/subscriptions/${plan.id}/deactivate`,
                                    {},
                                    { preserveScroll: true },
                                )
                            }
                        ...
                        <IconButton
                            ...
                            onClick={() =>
                                router.post(
                                    `/admin/subscriptions/${plan.id}/activate`,
                                    {},
                                    { preserveScroll: true },
                                )
                            }
    ...
    return (
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-4 lg:p-6">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                            Create and maintain platform plans and cashier
                            limits.
                        </p>
                    </div>
                </div>
                <form
                    onSubmit={submit}
                    className="grid gap-3 rounded-md border bg-card p-4 shadow-sm md:grid-cols-6"
                >
                    ...
                    <div>
                        ...
                        <Input
                            type="number"
                            min="1"
                            value={form.data.max_cashiers}
                            onChange={(event) =>
                                form.setData('max_cashiers', event.target.value)
                            }
                        />
                    </div>
                    <div>
